...
class AppController extends BaseController
{
    ...
    public function  index()
    {
        /* $fileName =str_random(50);                //定义文件名称
         ...
         $url = storage_path()."/app/public/image/0bR6oLqU0ZIzAtSEWuQsHG33SdGyvKoF5YK2Hq8ov6nocu78rN.png";
        ...
        // $s3Client = AwsFacade::createClient('s3', ["endpoint" => "https://digitaloceanspaces.com"]);
         $s3_return = $s3Client->putObject([
             'ACL'=>'public-read',
             'Bucket' => env("Bucket"), //存储桶（我的理解就是文件系统中的目录）
             'Key' => "test.txt", //文件名（包括后缀名）
             'Body' => "fdsjfkjdskfjdksfdsfdsfsd" //要上传的文件
         ]);
         dd($s3_return);*/
        ...
        if(empty($file) && $time) {
            ...
            if (is_dir($tmpDir)) {
                ...
                $s3_return = $s3Client->putObject([
                    ...
                    'Body' => file_get_contents($ipaFile) //要上传的文件
                ]);
    ...
    public function info($appName, $version=1.0, $id)
    {
        ...
        $app = App::findOrFail($id[0]);

        $qr = QrCode::size(100)->generate(env('APP_URL')."/app/".$app->id);
